...
def analyze_tuesday_to_wednesday_recovery_extended(klines):
    print("\n分析周二低点到周三高点反弹...")
    ...
    for tuesday_date, wednesday_date in tuesday_wednesday_pairs:
        ...
        # 计算各时段的表现
        sessions_analysis = analyze_sessions_in_period_extended(tuesday_data, wednesday_data, tuesday_low)
...
def analyze_sessions_in_period_extended(tuesday_data, wednesday_data, tuesday_low):
...
def generate_detailed_report_extended(analyses):
    """生成详细报告"""
    if not analyses:
        return "无分析结果"
    ...
    report = []
    report.append("=" * 70)
    report.append("BTC周二低点到周三高点反弹策略扩展分析报告")
    report.append("=" * 70)
    ...
    for i, analysis in enumerate(analyses, 1):
        report.append(f"\n案例 {i}: {analysis['tuesday_date']} 到 {analysis['wednesday_date']}")
        report.append(f"  周二低点: ${analysis['tuesday_low']:.2f} ({analysis['tuesday_low_time'][:16]})")
        report.append(f"  周三高点: ${analysis['wednesday_high']:.2f} ({analysis['wednesday_high_time'][:16]})")
    ...
    report.append(f"  实际验证结果: {overall_success_rate:.1f}% 成功率 ({target_achieved}/{total_pairs}次)")
    
    if overall_success_rate < 30:
        report.append(f"  ⚠️ 实际成功率远低于声称的69%，策略可能失效或需要调整")
    elif overall_success_rate < 50:
        report.append(f"  ⚠️ 实际成功率低于声称的69%，但仍有参考价值")
    else:
        report.append(f"  ✅ 实际成功率接近或超过声称的69%，策略有效")
...
def main():
    ...
    # 分析周二到周三反弹
    analyses = analyze_tuesday_to_wednesday_recovery_extended(klines)
    ...
    # 生成详细报告
    report = generate_detailed_report_extended(analyses)
    print("\n" + report)
    
    # 保存报告到文件
    with open('btc_tuesday_to_wednesday_extended_report.txt', 'w', encoding='utf-8') as f:
        f.write(report)
    
    # 保存详细数据到JSON
    with open('btc_tuesday_to_wednesday_extended_data.json', 'w', encoding='utf-8') as f:
        json.dump(analyses, f, ensure_ascii=False, indent=2, default=str)
    
    print(f"\n报告已保存到:")
    print(f"  - btc_tuesday_to_wednesday_extended_report.txt")
    print(f"  - btc_tuesday_to_wednesday_extended_data.json")
