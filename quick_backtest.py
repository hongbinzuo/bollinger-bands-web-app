...
def test_symbol_signals(symbol: str, strategy: MultiTimeframeStrategy):
    """测试单个币种的信号"""
    ...
    for timeframe in timeframes:
        try:
            ...
            logger.info(f"    {timeframe}: {len(pullback_signals)} 个信号")
...
def main():
    ...
    for i, symbol in enumerate(test_symbols):
        ...
        try:
            result = test_symbol_signals(symbol, strategy)
    ...
    for result in all_results:
        ...
        if total > 0:
            ...
            # 显示信号详情
            for signal in result.get('signals', [])[:3]:  # 只显示前3个
                profit = signal.get('profit_pct', 0)
                print(f"    {signal.get('timeframe')} {signal.get('signal')} EMA{signal.get('ema_period')} "
                      f"收益:{profit:.1f}%")
        else:
            print(f"  {symbol}: 无信号")
    ...
    if all_profits:
        ...
        print(f"\n收益率统计:")
        print(f"  平均收益率: {avg_profit:.1f}%")
        print(f"  最大收益率: {max_profit:.1f}%")
        print(f"  最小收益率: {min_profit:.1f}%")
        print(f"  有收益率的信号: {len(all_profits)} 个")
